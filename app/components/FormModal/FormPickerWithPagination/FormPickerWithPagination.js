...
const FormPickerWithPagination = (props) => {
    ...
    const { dataContent, edited, onChangeValue, fieldName, relatedObjectName, editRelations } = props;
    ...
    const {
        ...
        refetch,
        isFetching,
    } = usePaginatedQuery([relatedObjectName, pageNr], httpCT.fetchContentTypeObjects, {
        retry: 1,
        initialData: () => {
            const persistedData = contentRelationObjects && contentRelationObjects[relatedObjectName];
            if (persistedData && !netInfo.isInternetReachable) {
                return transformToArrayForListData(contentRelationObjects, relatedObjectName);
            }
        },
        onError: (err) => {
            const noPersistedData = !contentRelationObjects || !contentRelationObjects[relatedObjectName];
            if (netInfo.isInternetReachable) {
                ...
                if (err instanceof ApiNoDataError) {
                    dispatch(contentTypesActions.clearRelationsObject(relatedObjectName));
                    fetchingDataErrorAlert(err.message).then(() => {
                        queryCache.removeQueries(relatedObjectName);
            ...
            if (dataExists) {
                ...
                if (Array.isArray(lastData) && (lastData.length > 0)) {
                    ...
                    const newData = dat.data.map((el) => {
                        const labelFormat = transformToHumanRedeableTitle(el, partOfTitleProps);
                        const formattedLabel = (labelFormat !== el.id) ? `${labelFormat} - ${el.id}` : el.id;
                    ...
                    dispatch(contentTypesActions.setRelationsObjects(relatedObjectName, transformedData));
                    dispatch(contentTypesActions.setContentTypeObjectsTotalPages(relatedObjectName, totalPages));
                }
            }
            return dat;
        },
    ...
    const returnListData = useCallback(() => {
        const persistedData = contentRelationObjects && contentRelationObjects[relatedObjectName] && contentRelationObjects[relatedObjectName][pageNr - 1];
    ...
    const setInitPickerItem = useCallback(async (objName, keyName, editedRelations) => {
        if (!objName || !contentRelationObjects || !contentRelationObjects[relatedObjectName]) return null;
        ...
        if (edited) {
            ...
            const relatedDataUrl = relatedData && relatedData[0] && relatedData[0].dataUrl;
            relId = relatedDataUrl && relatedDataUrl.split('/').pop();
    ...
    const onSelectValueHandler = async (field, val, position = null, relatedObj = '', previewUrl = '', newLabel = '') => {
        if (!field) return;
                    ...
                    <View>
                        ...
                        {showPickerItem && (
                            <PickerItems
                                ...
                                onPressNext={() => (pageNr < getTotalPages()) && setPageNr(pageNr + 1)}
                            />
                        )}
                    </View>
                )
